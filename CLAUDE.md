...
## OAuth設定
- **Client ID**: `1051838677110-piv0se3763lgiock79vj641vcprkni43.apps.googleusercontent.com`
- **スコープ**: `https://www.googleapis.com/auth/calendar.events`
- **テストユーザー**: [redacted], [redacted], [redacted]
- Google Cloud Console でテストユーザーの追加・管理が必要

## デザイン方針
- **配色**: 緑(#6aab6d, #5aaa5e)＋クリーム(#fefcef) — みどりんの画像に合わせた配色
- **ヘッダー**: 背景色 `#6aab6d`（midorintop.pngの背景色と統一）
...
| ファイル | 用途 |
|---------|------|
| `midorintop.png` | ヘッダーアイコン |
| `midorin2.jpg` | ファビコン / PWAアイコン |
| `1月.jpg` 〜 `12月.jpg` | トップのマスコット画像（月ごとに自動切替） |
| `midorin-umbrella.jpg` | 中段デコレーション（mascot-between） |
| `midorin-tree.jpg` | 中段デコレーション |
| `midorin-basket.jpg` | 中段デコレーション |
| `midorin-clover.jpg` | フッター両端 / セクションラベルアイコン |
| `midorin-dango2.jpg` | セクションラベルアイコン（シフトを選択） |
| `yoko1.jpg` 〜 `yoko5.jpg` | フッター中央5つの画像 |

### 未使用（履歴的に残っているもの）
- `mascot-a〜d.svg` — 初期のSVGマスコット案（不採用）
- `midorin.jpg`, `midorin-front.jpg`, `midorin-side.jpg`, `midorin-back.jpg`, `midorin-faces.jpg` — 旧フッター画像
- `midorin-tea.jpg` — 旧トップ画像（月別画像に置き換え済み）
- `midorin-dango.jpg`, `midorin-clover-old.jpg` — 旧版（差し替え済み）
...
### UI/演出
8. **季節みどりん** — カレンダーの月切替で `{月}.jpg` のイラストに自動変更
